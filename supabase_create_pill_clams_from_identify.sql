-- Supabase SQL: Create and populate `pill_clams` from identify.html species array
-- Run this in Supabase SQL editor (or psql) to create the table and insert the species data
...
-- Insert data from the identify.html species array.
-- Note: "no image" and literal string "null" are mapped to SQL NULL.

INSERT INTO public.pill_clams (
  species, image_url, glossy, shape, striation_regular, posterior_point, callus_present, c4_shape, c2_shape, plica, ligament_long, umbo_taal, ligamentpit_shape, umbo_prominent, umbo_width, ligament_pit_width, laterals_developed, striation_strength, size_mm
) VALUES
...
-- Optional: Index and RLS policy (adjust as needed for your Supabase project)
CREATE INDEX IF NOT EXISTS idx_pill_clams_species ON public.pill_clams(species);

-- Enable RLS & allow public select if desired (ensure this matches your security model)
ALTER TABLE public.pill_clams ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "allow_public_select" ON public.pill_clams
  FOR SELECT
  TO public
  USING (true);

-- Refresh sequence in case of manual id inserts
SELECT setval(pg_get_serial_sequence('public.pill_clams','id'), coalesce(max(id),1)) FROM public.pill_clams;
