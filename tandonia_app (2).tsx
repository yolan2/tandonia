...
const Map = ({ onGridSelect, selectedGrid, onLocationSelect, mode }) => {
  ...
  useEffect(() => {
    ...
    if (selectedGrid && map.gridLayer) {
      ...
      const cell = map.gridCells.find(c => c.id === selectedGrid);
      if (cell) {
...
const ChecklistPage = ({ user }) => {
  ...
  useEffect(() => {
    const fetchSpecies = async () => {
      try {
        ...
        const initialCounts = mockSpecies.reduce((acc, sp) => ({ ...acc, [sp.id]: 0 }), {});
  ...
  const handleGridSelect = (gridId, bounds) => {
    setSelectedGrid(gridId);
    setGridBounds(bounds);
  };

  const handleLocationSelect = (type, latlng) => {
    setLocations(prev => ({ ...prev, [type]: latlng }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        const initialCounts = speciesList.reduce((acc, sp) => ({ ...acc, [sp.id]: 0 }), {});
        setSpecies(initialCounts);
        setTimeSpent('');
        setSubmitted(false);
      }, 3000);
                    ...
                    filteredSpecies.map(sp => (
                      <div key={sp.id} className="flex items-center justify-between p-3 bg-white rounded-lg border hover:border-green-500 transition">
                        ...
                        <input
                          ...
                          onChange={(e) => setSpecies(prev => ({ ...prev, [sp.id]: parseInt(e.target.value) || 0 }))}
